...
def load_registered_faces():
    ...
    for name, enc_blob in cursor.fetchall():
...
def save_new_face(known_encodings):
    ...
    if encodings:
        ...
        conn, cursor = connect_db()
        cursor.execute("INSERT INTO users (name, face_encoding) VALUES (%s, %s)", (name, pickle.dumps(encodings)))
        conn.commit()
        conn.close()
...
def main():
    ...
    while True:
        ...
        success = recognize_face(frame, known_encodings)
